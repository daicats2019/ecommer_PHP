...
class CartController extends Controller
{
            ...
            $output.='<ul class="hover-cart">';
                                    foreach(Session::get('cart') as $key => $value){
                                        $output.='<li><a href="#">
                                            ...
                                            <p>Quantity: '.$value['product_qty'].'</p>
                                        </a>
    ...
    public function check_coupon(Request $request){
        ...
        if(Session::get('customer_id')){
           $coupon = Coupon::where('coupon_code',$data['coupon'])->where('coupon_date_end','>=',$today)->where('coupon_used','LIKE','%'.Session::get('customer_id').'%')->first();
           if($coupon){
            return redirect()->back()->with('error','Discount code already used, please enter another code');
        }else{

           $coupon_login = Coupon::where('coupon_code',$data['coupon'])->where('coupon_date_end','>=',$today)->first();
                if($coupon_login){
                    ...
                    return redirect()->back()->with('error','The coupon code is incorrect - or has expired');
                }
        }
        //neu chua dang nhap
    }else{
        $coupon = Coupon::where('coupon_code',$data['coupon'])->where('coupon_date_end','>=',$today)->first();
        if($coupon){
...
public function gio_hang(Request $request){
    ...
    $slider = Slider::orderBy('slider_id','DESC')->where('slider_status','1')->take(4)->get();
     ...
     $meta_keywords = "Ajax Cart";
     $meta_title = "Ajax Cart";
    ...
    <table class="table table-condensed">
                    <thead>
                        <tr class="cart_menu">
                            ...
                            <td class="description">Amount in stock</td>
                    ...
                    if(Session::get('cart')==true){
                        ...
                        $output.='<tr>
                            ...
                            <td>';
                                ...
                                if(Session::get('customer_id')){
                                    $output.='<a class="btn btn-default check_out" href="'.url('/checkout').'">Order</a>';
                                }else{
                                    $output.='<a class="btn btn-default check_out" href="'.url('/log-in').'">Order</a>';
        ...
        foreach($data['cart_qty'] as $key => $qty){
            ...
            foreach($cart as $session => $val){
                ...
                if($val['session_id']==$key && $qty<$cart[$session]['product_quantity']){
                    ...
                    $message.='<p style="color:red">'.$i.') Update quantity :'.$cart[$session]['product_name'].' failure</p>';
                }
...
public function delete_all_product(){
    ...
    if($cart==true){
        ...
        return redirect()->back()->with('message','Successfully cleared all baskets');
    }
}
