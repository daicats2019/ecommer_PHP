...
class MailController extends Controller
{
	...
	public function send_coupon($coupon_time,$coupon_condition,$coupon_number,$coupon_code){
		...
		Mail::send('pages.send_coupon',  ['coupon'=>$coupon] , function($message) use ($title_mail,$data){
		 ...
		 return redirect()->back()->with('message','Sending promo code frequent customers successfully');
    }
	...
	public function send_coupon_vip($coupon_time,$coupon_condition,$coupon_number,$coupon_code){
		...
		return redirect()->back()->with('message','Successfully sent VIP customer promo code');
    }

    public function recover_pass(Request $request){
    	$data = $request->all();
		$now = Carbon::now('Asia/Ho_Chi_Minh')->format('d-m-Y');
		$title_mail = "Password recover ".' '.$now;
		...
		if($customer){
            ...
            if($count_customer==0){
                return redirect()->back()->with('error', 'Email is not registered to recover password');
            }else{
    ...
    public function reset_new_pass(Request $request){
        ...
        if($count>0){
                ...
                $reset->save();
                return redirect('log-in')->with('message', 'Password has been updated, please login');
        }else{
            return redirect('forgot-password')->with('error', 'Please re-enter your email as the link has expired');
        }
    }
